...
def get_muon_switchback_lr_profile(shape_class):
    ...
    return 0.84, 0.96, 0.74, get_muon_final_lr_frac(shape_class)

def get_muon_alpha(progress, shape_class):
    if shape_class == "rect":
        if progress < RECT_ALPHA_RECENTER_END:
            return lerp(RECT_ALPHA_RAW, RECT_ALPHA_BASE, phase_mix(progress, 0.0, RECT_ALPHA_RECENTER_END))
...
while True:
    ...
    for group in optimizer.param_groups:
        ...
        if group['kind'] == 'muon':
            ...
            elif group["shape_class"] == "square" and report_lrm_square is None:
                report_lrm_square = group["lr"] / group["initial_lr"]
                report_alpha_square = group["alpha"]
    report_lrm = report_lrm_rect if report_lrm_rect is not None else report_lrm_square
    report_alpha = report_alpha_rect if report_alpha_rect is not None else report_alpha_square
    ...
    print(f"\rstep {step:05d} ({pct_done:.1f}%) | loss: {debiased_smooth_loss:.6f} | lrm: {report_lrm:.2f} | alpha: {report_alpha:.2f} | accum: {current_grad_accum_steps} | dt: {dt*1000:.0f}ms | tok/sec: {tok_per_sec:,} | mfu: {mfu:.1f}% | epoch: {epoch} | remaining: {remaining:.0f}s    ", end="", flush=True)
