...
MUON_ALPHA_BASE = 0.50
MUON_ALPHA_OSC_AMPLITUDE = 0.018
MUON_ALPHA_OSC_START = 0.56
MUON_ALPHA_OSC_END = 0.84
MUON_LR_BREATH_AMPLITUDE = 0.06
MUON_LR_BREATH_START = 0.56
MUON_LR_BREATH_END = 0.84
...
def get_muon_breath_wave(progress, start, end):
    if progress < start or progress > end:
        return 0.0
    phase = phase_mix(progress, start, end)
    return math.sin(math.pi * phase) * math.sin(2.0 * math.pi * phase)

def get_muon_alpha(progress):
    return MUON_ALPHA_BASE + MUON_ALPHA_OSC_AMPLITUDE * get_muon_breath_wave(progress, MUON_ALPHA_OSC_START, MUON_ALPHA_OSC_END)

def get_muon_lr_breath(progress):
    return 1.0 + MUON_LR_BREATH_AMPLITUDE * get_muon_breath_wave(progress, MUON_LR_BREATH_START, MUON_LR_BREATH_END)
...
def get_group_lr_multiplier(group, progress):
    ...
    if kind == "muon" or subkind == "value_embed":
        base *= get_geometry_bank_scale(group, progress)
        if kind == "muon":
            base *= get_muon_lr_breath(progress)
        return base
...
while True:
    ...
    for group in optimizer.param_groups:
        ...
        if group['kind'] == 'muon':
            group["alpha"] = get_muon_alpha(progress)
            ...
            if report_lrm is None and group["shape_class"] == "square":
                ...
                report_alpha = group["alpha"]
    if report_lrm is None:
        report_lrm = 0.0
    if report_alpha is None:
        report_alpha = MUON_ALPHA_BASE
    optimizer.step()
    ...
    print(f"\rstep {step:05d} ({pct_done:.1f}%) | loss: {debiased_smooth_loss:.6f} | lrm: {report_lrm:.2f} | alpha: {report_alpha:.3f} | accum: {current_grad_accum_steps} | dt: {dt*1000:.0f}ms | tok/sec: {tok_per_sec:,} | mfu: {mfu:.1f}% | epoch: {epoch} | remaining: {remaining:.0f}s    ", end="", flush=True)
