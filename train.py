    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        for shape in sorted({p.shape for p in matrix_params}):
            ...
            param_groups.append(dict(
                ...
                muon_max_alpha=MUON_TAIL_MAX_ALPHA,
            ))
...
MUON_TAIL_START = 0.82
MUON_TAIL_FULL = 0.98
MUON_TAIL_MAX_ALPHA = 0.35
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_matrix_mix(self, group):
        ...
        muon_max_alpha = float(group.get("muon_max_alpha", MUON_TAIL_MAX_ALPHA))
        ...
        if "second_momentum_buffer" not in state:
            ...
            state["second_momentum_buffer"] = torch.zeros(state_shape, dtype=dtype, device=device)
        red_dim = -1 if shape[-2] >= shape[-1] else -2
        mix_frac = min(max((progress - muon_start) / max(muon_full - muon_start, 1e-6), 0.0), 1.0)
        mix_alpha = muon_max_alpha * mix_frac
        stacked_grads = torch.stack([p.grad for p in params])
        stacked_params = torch.stack(params)
        self._muon_momentum_t.fill_(group["momentum"])
        self._muon_beta2_t.fill_(group["beta2"] if group["beta2"] is not None else 0.0)
...
# Optimization
TOTAL_BATCH_SIZE = 196608 # record-branch moderate batch line
...
FINAL_LR_FRAC = 0.10    # keep the record branch's 10% LR floor intact
